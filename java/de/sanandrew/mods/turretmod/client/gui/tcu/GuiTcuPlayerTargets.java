        ...
        extends GuiTurretControlUnit
{
    ...
    public void initGui() {
        ...
        this.pagePlayerTargets.enabled = false;
    ...
    public void drawScreenPostBkg(int mouseX, int mouseY, float partTicks) {
        ...
        TmrClientUtils.doGlScissor(this.guiLeft + 6, this.guiTop + 19, this.xSize - 23, 115);
        ...
        for( Entry<UUID, Boolean> entry : this.tempTargetList.entrySet() ) {
            ...
            if( this.doSelectAll && !entry.getValue() ) {
                this.myTurret.getTargetProcessor().updatePlayerTarget(entry.getKey(), true);
                targetListChanged = true;
            } else if( this.doDeselectAll && entry.getValue() ) {
                this.myTurret.getTargetProcessor().updatePlayerTarget(entry.getKey(), false);
                targetListChanged = true;
            }
            ...
            if( mouseY >= btnMinOffY && mouseY < btnMaxOffY ) {
                if( mouseX >= this.guiLeft + 10 && mouseX < this.guiLeft + 18 && mouseY >= this.guiTop + 20 + offsetY && mouseY < this.guiTop + 28 + offsetY ) {
                    ...
                    if( isLmbDown && !this.prevIsLmbDown ) {
                        this.myTurret.getTargetProcessor().updatePlayerTarget(entry.getKey(), !entry.getValue());
                        targetListChanged = true;
                    }
                }
            }
            ...
            this.fontRendererObj.drawString(PlayerList.INSTANCE.getPlayerName(entry.getKey()), this.guiLeft + 20, this.guiTop + 20 + offsetY, textColor, false);
    ...
    private void updateTargets() {
        PacketRegistry.sendToServer(new PacketUpdateTargets(this.myTurret.getTargetProcessor()));
    }
    ...
    private void updateList() {
        ...
        btwSortMapNm.putAll(this.myTurret.getTargetProcessor().getPlayerTargets());
