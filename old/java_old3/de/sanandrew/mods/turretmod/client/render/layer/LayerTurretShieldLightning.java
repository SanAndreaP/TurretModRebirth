...
import de.sanandrew.mods.turretmod.client.render.RenderStateVals;
        ...
        extends LayerRenderer<E, M>
{
    ...
    private static final RenderType LIGHTNING_TYPE = RenderType.makeType("tf_lightning", DefaultVertexFormats.POSITION_COLOR, 7, 256, false, true,
                                                                         RenderType.State.getBuilder()
                                                                                         .writeMask(RenderStateVals.COLOR_DEPTH_WRITE)
                                                                                         .transparency(RenderStateVals.LIGHTNING_TRANSPARENCY)
                                                                                         .shadeModel(RenderStateVals.SHADE_ENABLED)
                                                                                         .lightmap(RenderStateVals.LIGHTMAP_DISABLED)
                                                                                         .build(false));
                       ...
                       float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch)
    {
        ...
        if( turretInst.isActive() ) {
            if( MiscUtils.RNG.randomInt(150) == 0 && !Minecraft.getInstance().isGamePaused() ) {
...
//                        builder.
                        ...
                        for( int k1 = 0; k1 < 5; k1++ ) {
                            ...
                            twigBuffer.pos(xTwigMin + minX,    level * 16,       zTwigMin + minZ)   .color(0.9F * lum, 0.8F * lum, 0.1F * lum, alpha).lightmap(0, 0xF0).endVertex();
                            twigBuffer.pos(xTwigMax + xBranch, (level + 1) * 16, zTwigMax + zBranch).color(0.9F * lum, 0.8F * lum, 0.1F * lum, alpha).lightmap(0, 0xF0).endVertex();
                        }
