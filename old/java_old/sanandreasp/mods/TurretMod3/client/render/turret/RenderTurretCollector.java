...
public class RenderTurretCollector extends RenderTurret_Base {
	...
	public void doRender(EntityLiving par1EntityLiving, double par2, double par4, double par6, float par8, float par9) {
        ...
        GL11.glPushMatrix();
        GL11.glTranslatef((float)par2, (float)par4+2.05F, (float)par6);
        ...
        for (int j = 0; j < 4; j++)
        {
            ...
            for (int k = 0; k < 3; k++)
            {
                ...
                for (int j1 = l-0; j1 >= i1; j1--)
                {
                    ...
                    float f = 0.5F;
                    tessellator.setColorRGBA_F(0.2F * f, 0.9F * f, 0.2F * f, 0.3F);
    ...
    protected void renderStats(EntityTurret_Base par1Turret, double par2, double par4, double par6)
    {
        if (Minecraft.isGuiEnabled() && TM3ModRegistry.proxy.getPlayerTM3Data(Minecraft.getMinecraft().thePlayer).getBoolean("renderLabels") && !par1Turret.isInGui())
        {
            ...
            if (var10 < (double)(var12 * var12))
            {
                ...
                var15.addVertex(exp, 11.5D, 0D);
                var15.addVertex(exp, 13.5D, 0D);
